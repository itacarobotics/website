...
In this tutorial we will explore how different `Kp` and `Kd` values of a PD controller influence the behavior of the robot. We will start by discussing what happens under the hood, and then experiment directly on the hardware.
...
To change the `Kp` and `Kd` values, open the file `ulixarm.ros2_control.xacro`, located at:
...
:::danger
Do **not** set `Kp` or `Kd` values outside the boundaries listed in the table below. Out-of-range values can cause joint instability and unpredictable robot motion.
